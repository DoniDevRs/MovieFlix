...
final public class MFMovieDetailView: UIView {
    ...
    private lazy var errorView: MFErrorView = {
        let errorView = MFErrorView()
        errorView.translatesAutoresizingMaskIntoConstraints = false
        errorView.delegate = self
        return errorView
    }()
    
    private lazy var contentError: UIView = {
        let contentError = UIView()
        contentError.translatesAutoresizingMaskIntoConstraints = false
        contentError.isHidden = true
        return contentError
    }()
    ...
    private func buildViewHierarchy() {
        ...
        view.addSubview(contentError)
        contentError.addSubview(errorView)
    ...
    private func addConstraints() {
        ...
        contentError.constraintToSuperView()
        errorView.constraintToSuperView(top: -Metrics.giant,
    ...
    private func updateViewError(show: Bool,
                                 entity: ErrorViewEntity? = nil) {
        if show { errorView.updateView(with: entity ?? ErrorViewEntity.makeCustomError()) }
        contentError.isHidden = !show
...
extension MFMovieDetailView: MFMovieDetailProtocol {
    public func updateView(with viewState: MFMovieDetailViewState) {
        ...
        case let .hasError(error):
            updateViewError(show: true, entity: error)
        case .isEmpty:
            break
        }
    }
...
extension MFMovieDetailView: ErrorViewDelegate {
