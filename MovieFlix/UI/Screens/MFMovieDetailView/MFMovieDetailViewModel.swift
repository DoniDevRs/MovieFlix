...
public class MFMovieDetailViewModel: MFMovieDetailViewModelProtocol {
    ...
    typealias UseCaseGetMovieDetailUseCase = Result<GetMovieDetailUseCaseResponse, NetworkingError>
    
    private let getMovieCreditsUseCase: GetMovieCreditsUseCaseProtocol
    typealias UseCaseGetMovieCreditsUseCase = Result<GetMovieCreditsUseCaseResponse, NetworkingError>
    
    private let getMovieVideoUseCase: GetMovieVideoUseCaseProtocol
    typealias UseCaseGetMovieVideoUseCase = Result<GetMovieVideoUseCaseResponse, NetworkingError>
    ...
    public init(getMovieDetailUseCase: GetMovieDetailUseCaseProtocol, 
                getMovieCreditsUseCase: GetMovieCreditsUseCaseProtocol,
                getMovieVideoUseCase: GetMovieVideoUseCaseProtocol,
                movieId: Int) {
    ...
    private func handleGetMovieDetail(_ result: UseCaseGetMovieDetailUseCase ) {
        ...
        case let .failure(error):
            let errorEntity = ErrorViewEntity(title: ErrorViewEntity.Constants.title,
                                              description: error.localizedDescription,
                                              buttonTitle: ErrorViewEntity.Constants.buttonTitle)
            viewState = .hasError(errorEntity)
        }
    }
    ...
    private func getMovieURL(_ movieResponse: GetMovieVideoUseCaseResponse) {
        ...
        movie.forEach { movie in
            ...
            guard movie.name == "Official Trailer" else {
                return
            }
            var youtubeURL: URL? {
